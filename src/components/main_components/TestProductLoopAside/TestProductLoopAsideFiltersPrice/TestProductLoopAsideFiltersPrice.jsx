...
import classes from "./TestProductLoopAsideFiltersPrice.module.css";
...
const TestProductLoopAsideFiltersPrice = () => {
  const locationState = useLocation();
  const { productsList, filterProductsList } = useTypedSelector(
    (state) => state
  );
  ...
  const handlePriceChange = (event, newPriceValue) => {
    setPriceFilterValue(newPriceValue);
  };
  ...
  useEffect(() => {
    if (productArray.length !== 0) {
      setMaxPrice(
        Math.round(Math.max(...productsList.map((product) => product.price)))
      );
    }
  }, [productArray]);
  ...
  useEffect(() => {
    addProductsToFilterList(
      productsList.filter(
        (product) =>
          product.price < priceFilterValue[1] &&
          product.price > priceFilterValue[0]
      )
    );
  }, [priceFilterValue]);

  useEffect(() => {
    setPriceFilterValue([0, Math.round(maxPrice)]);
  }, [maxPrice]);
...
export default TestProductLoopAsideFiltersPrice;
