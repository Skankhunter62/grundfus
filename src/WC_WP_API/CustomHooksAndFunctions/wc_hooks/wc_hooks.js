...
export const addWCOrder = async (WC_Order, setOrderData, changeSetLoading) => {
    changeSetLoading()
    await axios.post(wcBaseUrl + 'orders', WC_Order, data)
        .then(response => {if(response.status === 201){setOrderData(response.data)} else {console.log('Problems with server connection, when try send order request')} })
}

/*------------------Product Reviews Block Hooks --------------------------------*/
/*send product review*/
export const addWCReview = async (WC_Review, setReviewData) => {
    await axios.post(wcBaseUrl + 'products/reviews', WC_Review, data)
        .then(response => {if(response.status === 201){setReviewData(response.data)} else {console.log('Problems with server connection, when try send product review')} })
}
/*get reviews from single product*/
export const useWCProductReview = (productId, setReviewData) => {
    useEffect(()=>{
        reviewsApi.get(`products/${productId}/reviews`)
            .then(response => {if(response.status === 200){setReviewData(response.data)} else {console.log('Problems with server connection, when try get product review')} })
    }, [productId])
}
...
export const getWCProductBySingleCategory = (categoryId, addProducts, addProductsAttributes) => {
    ...
    api.get(`products?category=${categoryId}`)
        .then(response => {
            ...
            else{console.log('Problems with server connection, when try get products by category')}
        })
}
    ...
    useEffect(() => {
        api.get('products?status=publish')
            .then((response) => {
                ...
                else{console.log('Problems with server connection, when try get all products')}
            })
    }, [])
}
    ...
    useEffect(() => {
        api.get('products?orderby=popularity&order=desc&per_page=10')
            .then(response => {
                ...
                else{console.log('Problems with server connection, when try get popular products')}
            })
    }, [])
    return popularGoods;
}
    ...
    useEffect(() => {
        api.get('products?orderby=date&order=desc&per_page=10')
            .then(response => {
                ...
                else{console.log('Problems with server connection, when try get new products')}
            })
    }, [])
    return newGoods;
}
